...
namespace Framework2D {
	...
	SpriteEntity::SpriteEntity(const std::string& Name, Texture* texture, const Vec2Int& Position,
	...
	{
		if (Size == Vec2Int::VecZero)
			this->Size = { texture->GetWidth(), texture->GetHeight() };
	}

	SpriteEntity::SpriteEntity(const std::string& Name, const std::string& TexturePath, const Vec2Int& Position,
		const Vec2Int& Size, const Vec4& Color, bool bEnableRender)
